...
def calculateContainment(
    ...
    # If no genes have a valid CAG label, stop the analysis
    if aln_df["CAG"].isnull().sum() == aln_df.shape[0]:
        print("Stopping the analysis")
        return

    # Implicitly, at least some genes have CAGs
    aln_df = aln_df.assign(
    ...
    if len(genome_containment) == 0:
        print("No matching genomes, stopping")
        return
    ...
    genome_containment_df["CAG"] = genome_containment_df["CAG"].apply(
        int).apply(str)

    genome_containment_df.to_csv(
        # File name will end with .csv.gz
        f"genome_containment_shard.{header_csv_gz}",
        sep=",",
        compression="gzip",
        index=None
    )
