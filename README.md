...
over a set of microbial genomes by:
  ...
  2. Calculating summary metrics for each genome using both the consistency and magnitude of assocation for the genes it contains, and
...
AMGMA outputs are also written to a redis database to help support rapid visualization.
The keys populated in that database are as follows:

* `genome_name`: Dict with `<genome index>:<human readable name>`
* `genome_acc`: Dict with `<genome index (int)>:<unique accession ID (string)>`
* `genome_cag_assignments <genome_id>`: Dict with `<cag_id>:<number of aligned genes>`
* `cag_genome_assignments <cag_id>`: Dict with `<genome_id>:<number of aligned genes>`
* `n_genes_per_genome`: Dict with `<genome index>:<# of genes aligned>`
* `genome_association <PARAM>`: DataFrame with `genome_id, estimate, p_value, std_error, q_value, wald`
* `gene_name`: Dict with `<gene_ix>:<gene name>`
* `gene_cag_id`: Dict with `<gene_ix>:<CAG ID>`
* `gene_tax_id`: Dict with `<gene_ix>:<tax ID>`
* `gene_func_id`: Dict with `<gene_ix>:<function index>`
* `genome_tax_assignments <genome_id>`: DataFrame with the number of genes assigned to each tax ID
* `tax_genome_assignments <tax_id>`: Dict with `<genome_id>:<number of aligned genes>`
* `genome_contigs <genome_id>`: DataFrame with the name and length of each contig
* `genome_alignment <genome_id>`: DataFrame with the position and index for every aligned gene
* `genome_abundance_specimen <genome_id>`: Series with `<specimen>:<relative abundance of genome>`
* `specimen_abundance_genome <specimen>`: Series with `<genome_id>:<relative abundance of genome>`
